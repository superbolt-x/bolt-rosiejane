...
WITH paid_data as
    ...
    FROM
        ...
        WHERE campaign_name ~* 'sephora' AND campaign_id = 23852671081560313
        UNION ALL
        ...
        UNION ALL
        SELECT 'TikTok DTC Prospecting' as channel, date, date_granularity, 
            spend, clicks, impressions, purchases, revenue
        FROM {{ source('reporting','tiktok_ad_performance') }}
        WHERE campaign_name !~* 'traffic' and campaign_name ~* 'prospecting'
        UNION ALL
        SELECT 'TikTok DTC Retargeting' as channel, date, date_granularity, 
            spend, clicks, impressions, purchases, revenue
        FROM {{ source('reporting','tiktok_ad_performance') }}
        WHERE campaign_name !~* 'traffic' and campaign_name ~* 'retargeting'
        UNION ALL
        ...
        UNION ALL
        SELECT 'Tiktok GMV' as channel, date_trunc('day',date) as date, 'day' as date_granularity, 
        ...
        GROUP BY 1,2,3
        UNION ALL
        SELECT 'Tiktok GMV' as channel, date_trunc('week',date) as date, 'week' as date_granularity, 
            sum(spend) as spend, sum(0) as clicks, sum(0) as impressions, sum(purchases) as purchases, sum(revenue) as revenue
        FROM {{ source('gsheet_raw','tiktok_gmv_campaigns_insights') }}
        GROUP BY 1,2,3
        UNION ALL
        SELECT 'Tiktok GMV' as channel, date_trunc('month',date) as date, 'month' as date_granularity, 
            sum(spend) as spend, sum(0) as clicks, sum(0) as impressions, sum(purchases) as purchases, sum(revenue) as revenue
        FROM {{ source('gsheet_raw','tiktok_gmv_campaigns_insights') }}
        GROUP BY 1,2,3
        UNION ALL
        SELECT 'Tiktok GMV' as channel, date_trunc('quarter',date) as date, 'quarter' as date_granularity, 
            sum(spend) as spend, sum(0) as clicks, sum(0) as impressions, sum(purchases) as purchases, sum(revenue) as revenue
        FROM {{ source('gsheet_raw','tiktok_gmv_campaigns_insights') }}
        GROUP BY 1,2,3
        UNION ALL
        SELECT 'Tiktok GMV' as channel, date_trunc('year',date) as date, 'year' as date_granularity, 
            sum(spend) as spend, sum(0) as clicks, sum(0) as impressions, sum(purchases) as purchases, sum(revenue) as revenue
        FROM {{ source('gsheet_raw','tiktok_gmv_campaigns_insights') }}
        GROUP BY 1,2,3
        )
